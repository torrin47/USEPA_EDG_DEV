...
public class RestQueryServlet extends BaseServlet {
  ...
  protected void execute(HttpServletRequest request,
    ...
    try {
      ...
      getLogger().finer("Handling rest query string="+request.getQueryString()+" Result:"+query.toString());
    } catch (Throwable t) {
    ...
    try {
      ...
      if(feedWriter instanceof HtmlAdvancedWriter) {
        ((HtmlAdvancedWriter)feedWriter).write(result);
          ...
          //Added by Baohong
          SearchResultRecordsAdapter searchResultRecordsAdapter = new SearchResultRecordsAdapter(result.getRecords());
          feedWriter.write(searchResultRecordsAdapter);
 ...
 // make the search engine
    ...
    ResourceLinkBuilder rBuild = ResourceLinkBuilder.newBuilder(context,request, messageBroker);
  ...
  protected RestQuery parseRequest(HttpServletRequest request, RequestContext context) {
    ...
    parser.parseMaxRecords("max",10);
//    parser.parsePropertyIsEqualTo("uuid","uuid");
    parser.parsePropertyList("uuid","uuid",",",true);
    parser.parsePropertyIsLike("searchText","anytext");
    parser.parsePropertyList("contentType","dc:type",",",true);
    parser.parsePropertyList("dataCategory","dc:subject",",",true);
    parser.parsePropertyRange("after","before","dct:modified");
    parser.parseSpatialClause("bbox","spatialRel","geometry");
    parser.parseSortables("orderBy");
    //parser.parsePropertyIsEqualTo("owner","owner");
...
//    parser.parsePropertyList("collectionlist","dc:collectionlist",",",true);
    
   //changes made by NETTY
    parser.parsePropertyList("collection","dc:collection",",",true);
    
    //parser.parsePropertyIsLike("childlist","childlist");
    //parser.parsePropertyIsLike("parentlist","parentlist");
    //parser.parsePropertyIsLike("collectionlist","collectionlist");
    return query;
  }
