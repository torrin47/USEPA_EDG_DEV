...
class ImsMetadataProxyDao extends BaseDao {
  ...
  protected int insertRecord(PutMetadataRequest request, PutMetadataInfo info)
    ...
    String schemaKey = info.getSchemaKey();
    ...
    try {
      ...
      if (id<0) {
        ...
        sql.append("TITLE,");
        sql.append("OWNER,");
        sql.append("SCHEMA_KEY");  // JSS 20110325, to make schema key form schema def file available
        sql.append(")");
        sql.append(" VALUES(?,?,?,?)");  // JSS 20110325, to make schema key form schema def file available
  ...
  protected int transferOwnership(TransferOwnershipRequest request, String uuid, String newOwner)
    ...
    try {
      ...
      // JSS change update date to force re-indexing
      sql.append(" SET OWNER=?, UPDATEDATE=current_timestamp WHERE DOCUUID=?");
      logExpression(sql.toString());
      Connection con = returnConnection().getJdbcConnection();
      st = con.prepareStatement(sql.toString());
      st.setInt(1,ownerID);
