...
public class ResourceLinkBuilder {
    ...
    private final String parentFindPath = "/rest/find/document?xsl=metadata_to_html_full&f=html";
    private final Map<String, String> labels = new HashMap<String, String>();
    private MessageBroker messageBroker;
    private RequestContext requestContext;
    private ResourceIdentifier resourceIdentifier;
    private innoCollection innoCollectionObj;
    private HashMap resourceParentChildChk = null;
...
//        this.buildPreviewLink(xRecord, record);
        ...
        this.buildParentLink(xRecord, record);
        this.buildChildLink(xRecord, record);
        this.buildVisualizeCollectionTree(xRecord, record);
    }
    ...
    protected void buildAddToMapLink(SearchXslRecord xRecord, SearchResultRecord record) {
        ...
        if ((viewerUrl.length() != 0)) {
            ...
            if (resourceUrl.length() == 0 || (serviceType.length() == 0)) {
                ...
                boolean canHandle = (serviceType.equalsIgnoreCase(ServiceType.AGS.name())
                        ...
                        || serviceType.equalsIgnoreCase(ServiceType.WFS.name()) || serviceType.equalsIgnoreCase(ServiceType.WCS.name()));
                if (!canHandle) {
                    return;
                }
            }
        ...
        if ((resourceUrl.toLowerCase().startsWith("http:") || resourceUrl.toLowerCase().startsWith("https:"))
            ...
            if (serviceType.equals("ags")) {
                resourceUrl = Val.chkStr(this.getResourceIdentifier().guessAgsMapServerSoapUrl(resourceUrl));
            }
    ...
    protected void buildAGSLinks(SearchXslRecord xRecord, SearchResultRecord record) {
        String resourceUrl = Val.chkStr(record.getResourceUrl());
        String serviceType = Val.chkStr(record.getServiceType()).toLowerCase();
        String restUrl = Val.chkStr(this.getResourceIdentifier().guessAgsServiceRestUrl(resourceUrl));
        String url;
        String resourceKey;
        ResourceLink link;
    ...
    protected void buildContentTypeLink(SearchXslRecord xRecord,
        ...
        if (contentType.length() > 0) {
            contentType = this.getResourceIdentifier().guessArcIMSContentTypeFromResourceType(contentType);
        }
        if (contentType.length() == 0) {
     ...
     * @param record the search result record
     */
    protected void buildParentLink(SearchXslRecord xRecord, SearchResultRecord record) {

        String uuid = Val.chkStr(record.getUuid());
        String url = "";

        if (this.resourceParentChildChk.containsKey(uuid)) {
            HashMap chk = (HashMap) this.resourceParentChildChk.get(uuid);
            url = this.getBaseContextPath() + this.parentFindPath + "&parentof=" + encodeUrlParam(uuid);
           
            if (chk.get("hasparent").toString().equals("1")) {
                String resourceKey = "catalog.rest.viewParent";
                ResourceLink link = this.makeLink(url, "Parent", resourceKey);
                record.getResourceLinks().add(link);
            }
        }
    ...
    protected void buildChildLink(SearchXslRecord xRecord, SearchResultRecord record) {
        ...
        if (this.resourceParentChildChk.containsKey(uuid)) {
            HashMap chk = (HashMap) this.resourceParentChildChk.get(uuid);
            url = this.getBaseContextPath() + this.parentFindPath + "&childrenof=" + encodeUrlParam(uuid);
    

            if (chk.get("haschild").toString().equals("1")) {
                String resourceKey = "catalog.rest.viewChild";
                ResourceLink link = this.makeLink(url, "Children", resourceKey);
                record.getResourceLinks().add(link);
            }
        }
    }
    
    protected void buildVisualizeCollectionTree(SearchXslRecord xRecord, SearchResultRecord record) {
        String uuid = Val.chkStr(record.getUuid());
        String url = "";
        String col_id = "";
        ...
        ServletRequest sRequest = rContext.getServletRequest();
        String f=sRequest.getParameter("f");
        
        if(f==null){
            f=(String) sRequest.getAttribute("f");
        }
        
        ic = new innoCollection(RequestContext.extract(sRequest));
        ResultSet rst = ic.fnCollection(uuid);
        try {
            while (rst.next()) {
                if (rst.getString("col_id") != null) {
                    col_id = rst.getString("col_id");
                }
            }
        } catch (SQLException ex) {
            Logger.getLogger(ResourceLinkBuilder.class.getName()).log(Level.SEVERE, null, ex);
        }

        if (col_id != "") {
            
            if("html".equals(f) ||"htmlfragment".equals(f)||"searchpageresults".equals(f)){
                  url = "javascript:openTreeChart('" + col_id + "')";
            }else{
                url= this.getBaseContextPath()+ "/catalog/search/visualizeCollection.jsp?"+col_id;
            }

            String resourceKey = "catalog.rest.viewVisualizeCollection";
            ResourceLink link = this.makeLink(url, "self", resourceKey);
            
            if("georss".equals(f) || "kml".equals(f)){
  ...
  /*
    ...
    protected void determineResourceUrl(SearchXslRecord xRecord,
        ...
        if ((resourceUrl.length() > 0)
                ...
                || serviceType.equalsIgnoreCase("feature") || serviceType.equalsIgnoreCase("metadata"))) {
    ...
    public void initialize(HttpServletRequest request, RequestContext context,MessageBroker messageBroker) {
        ...
        if (this.resourceParentChildChk == null) {
            try {
                this.innoCollectionObj = new innoCollection(context);
                this.resourceParentChildChk = this.innoCollectionObj.getResourceParentChildChk();
                
                //LOG.log(Level.SEVERE, "Deewendra " + this.resourceParentChildChk);
            } catch (Exception ex) {
     ...
     */
    public static ResourceLinkBuilder newBuilder(RequestContext context,HttpServletRequest servletRequest, MessageBroker messageBroker) {
        ...
        String className = Val.chkStr(catCfg.getParameters().getValue("resourceLinkBuilder"));
